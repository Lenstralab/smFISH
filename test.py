...
def make_test_FISH_pipeline(parameter_file):
    def pipeline_fun(tmp_path):
        ...
        for file in parameters['fileListIn']:
            tmp_paramsfile = get_exp_info(file, parameters)
            tmp_lfn = get_filelist(file, parameters)
            ...
            for fn in tmp_lfn:
                for f in files:
                    assert os.path.exists(path := os.path.join(tmp_paramsfile['pathOut'],
                                            f"{tmp_paramsfile['date']}_{fn}{f}")), f'File {path} has not been generated'
